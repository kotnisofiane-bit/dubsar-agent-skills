...
function renderSummary(documents, validation) {
  ...
  const evidence = documents["evidence.json"].entries;
  const supportedEvidence = evidence
    .filter((entry) => ["observed", "derived"].includes(entry.class))
    .map((entry) => `[${entry.class}] ${entry.claim} (${entry.evidence_id})`);
  const unresolvedEvidence = evidence
    .filter((entry) => ["reported", "unverified"].includes(entry.class))
    .map((entry) => `[${entry.class}] ${entry.claim} (${entry.evidence_id})`);
